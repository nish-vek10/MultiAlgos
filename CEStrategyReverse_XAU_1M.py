"""
Chandelier Exit Strategy for XAUUSD (Reverse) - M1
Using Heikin-Ashi Candles
XAU - 1 Minute TF
"""

# ===== FILENAME: CEStrategyReverse_XAU_1M =====
...
atr_period = 1
atr_mult = 1.85
magic_number = 223344  # Unique ID for this EA's trades (reverse bot)
local_tz = timezone('Europe/London')

# === RISK & STOPS CONFIG === #
risk_per_trade_pct = 0.25      # percent of balance risked per trade
sl_usd_distance    = 2.50      # fixed $ distance from entry (price dollars)

# === SESSION FILTER (Europe/London local time) === #
# M1 mean-reversion windows (avoid London/NY trend core, trim edges)
session_windows = [
    ("00:30", "05:50"),
    ("18:05", "21:50"),
]
...
# === ACCOUNT LOGIN CONFIG (separate login recommended for reverse) === #
mt5_login = 52474878
...
def send_order(symbol, action_type, lot=None):
    ...
    request = {
        ...
        "deviation": slippage,
        "magic": magic_number,
        "comment": "ChandelierReverseBot_M1",
...
while True:
    ...
    print("\n= = = LAST 10 HEIKIN-ASHI CANDLES WITH SIGNALS (REVERSE) = = =")
    debug_df = tr[['ha_c', 'ha_open', 'ha_high', 'ha_low', 'dir', 'buy_signal', 'sell_signal']].copy()
    debug_df.index = debug_df.index.strftime('%Y-%m-%d %H:%M')
    # show original signal column for visibility
    debug_df['orig_signal'] = debug_df.apply(lambda row: 'BUY' if row['buy_signal'] else ('SELL' if row['sell_signal'] else ''), axis=1)
    print(debug_df[['ha_c', 'ha_open', 'ha_high', 'ha_low', 'dir', 'orig_signal']].tail(10))

    # REVERSE: invert what the CE says
    signal = None
    ...
    # 7) execute according to REVERSED signals
    if signal == 'BUY':
        if open_position == 'SELL':
            close_position(position, mt5_symbol)
        if open_position != 'BUY':
            print("[EXEC-REV] REVERSED BUY (orig SELL) → sending order.")
        ...
        if open_position == 'BUY':
            close_position(position, mt5_symbol)
        if open_position != 'SELL':
            print("[EXEC-REV] REVERSED SELL (orig BUY) → sending order.")
