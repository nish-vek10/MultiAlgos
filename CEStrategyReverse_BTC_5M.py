"""
Chandelier Exit Strategy for BTCUSD (Reverse)
Using Heikin-Ashi Candles
BTC - 5 Minute TF
"""

# ===== FILENAME: CEStrategyReverse_BTC_5M =====
...
atr_period = 1
atr_mult = 1.85
magic_number = 110011  # Unique ID for this EA's trades
local_tz = timezone('Europe/London')

# === RISK & STOPS CONFIG === #
risk_per_trade_pct = 0.25      # percent of balance risked per trade
sl_usd_distance    = 150.0       # fixed $ distance from entry


# === SESSION FILTER (Europe/London local time) === #
session_windows = [
    ("01:00", "06:00"),  # Pre-London / Asian hours (choppy/mean-reverting)
    ("22:00", "00:00"),  # After overlap fades until late US evening
]
...
mt5_login = 52526821
mt5_password = "XALg2O!dZ&j3XO"
mt5_server = "ICMarketsSC-Demo"
mt5_terminal_path = r"C:\MT5\52526821\terminal64.exe"
...
def fetch_oanda_candles(symbol=oanda_symbol, granularity="M5", count=num_candles):
    ...
    symbol: OANDA instrument name (e.g., "BTC_USD")
...
def compute_lot_for_risk(symbol, sl_usd, risk_pct):
    ...
    # 4) Last resort: contract size (works for BTCUSD quoted in USD)
    if value_per_1usd_per_lot is None:
...
def send_order(symbol, action_type, lot=None):
    """
    Send a buy or sell market order sized by risk and with fixed $2.5 SL, no TP.
...
def _maybe_retry_pending():
    ...
    (countdowns, data waits, outside the 'new candle' moments). Respects session windows.
    """
    ...
    print(f"[RETRY] BTC-5M-REV: attempting pending {pending_signal} execution...")
    ok = _attempt_execution_for_signal(pending_signal)
    last_retry_at = now_local
    if ok:
        print(f"[OK] BTC-5M-REV: pending {pending_signal} executed.")
        pending_signal = None
        pending_since  = None
        last_retry_at  = None
...
pending_signal = None        # 'BUY' or 'SELL' still to be executed
pending_since  = None        # datetime when it was queued
last_retry_at  = None        # last retry timestamp
RETRY_EVERY_SECS = 15        # throttle while waiting (safe with 0.25–1.0s loops)
...
while True:
    ...
    # decide signal (REVERSE MODE)
    signal = 'SELL' if latest['buy_signal'] else ('BUY' if latest['sell_signal'] else None)

    # previous-bar signal for backfill (REVERSE mapping)
    prev_signal = None
    if len(tr) >= 2:
        prev = tr.iloc[-2]
        if prev['buy_signal']:
    ...
    # execute according to signals (REVERSE) with pending/backfill/override
    now_local = datetime.now(local_tz)

    # Guard again (protects if session ended between calc and here)
    if now_local >= current_session_end:
        print("[INFO] SESSION ENDED BEFORE TRADE EXECUTION (post-calc guard).")
        if auto_close_at_session_end:
            pos = get_position(mt5_symbol)
    ...
    if signal:
        ...
        if pending_signal and signal != pending_signal:
            print(f"[CANCELLED] BTC-5M-REV: live signal {signal} overrides pending {pending_signal}.")
            ...
            if signal != last_signal or open_position != signal:
                ...
                print(f"[TRADE] BTC-5M-REV: new signal={signal} | prev={prev_sig} | open={open_position or 'NONE'}")
                last_signal = signal
                ok = _attempt_execution_for_signal(signal)
                if ok:
                    pending_signal = None
                    ...
                    if not pending_signal:
                        pending_signal = signal
                        pending_since = now_local
                        print(f"[PENDING] BTC-5M-REV: queued {signal} execution (will retry).")
    ...
    # 5B) No fresh latest signal: backfill previous bar if it looks missed
        ...
        if prev_signal and (last_signal != prev_signal or open_position != prev_signal):
            if not pending_signal:
                print(f"[BACKFILL] BTC-5M-REV: previous bar had {prev_signal} — attempting execution now.")
                ...
                if ok:
                    ...
                    pending_since = now_local
                    print(f"[PENDING] BTC-5M-REV: queued {prev_signal} from previous bar (will retry).")
        else:
