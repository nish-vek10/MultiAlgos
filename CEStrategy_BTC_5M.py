"""
Chandelier Exit Strategy for BTCUSD
Using Heikin-Ashi Candles
BTC - 5 Minute TF
"""

# ===== FILENAME: CEStrategy_BTC_5M =====
...
atr_period = 1
atr_mult = 1.85
magic_number = 110022  # Unique ID for this EA's trades
local_tz = timezone('Europe/London')

# === RISK & STOPS CONFIG === #
risk_per_trade_pct = 0.25      # percent of balance risked per trade
sl_usd_distance    = 250.0       # fixed $ distance from entry


# === SESSION FILTER (Europe/London local time) === #
session_windows = [
    ("10:00", "13:00"),  # London morning
    ("14:00", "20:00"),  # US session + London/NY overlap
]
...
mt5_login = 52526820
mt5_password = "g9RCU8W&iBJzgv"
mt5_server = "ICMarketsSC-Demo"
mt5_terminal_path = r"C:\MT5\52526820\terminal64.exe"
...
def send_order(symbol, action_type, lot=None):
    """
    Send a buy or sell market order sized by risk and with fixed $500.00 SL, no TP.
...
def _maybe_retry_pending():
    ...
    print(f"[RETRY] BTC-5M: attempting pending {pending_signal} execution...")
    ok = _attempt_execution_for_signal(pending_signal)
    last_retry_at = now_local
    if ok:
        print(f"[OK] BTC-5M: pending {pending_signal} executed.")
        pending_signal = None
        pending_since  = None
        last_retry_at  = None
...
while True:
    ...
    if signal:
        ...
        if pending_signal and signal != pending_signal:
            print(f"[CANCELLED] BTC-5M: live signal {signal} overrides pending {pending_signal}.")
            ...
            if signal != last_signal or open_position != signal:
                ...
                print(f"[TRADE] BTC-5M: new signal={signal} | prev={prev_sig} | open={open_position or 'NONE'}")
                last_signal = signal
                ...
                if ok:
                    ...
                    if not pending_signal:
                        pending_signal = signal
                        pending_since = now_local
                        print(f"[PENDING] BTC-5M: queued {signal} execution (will retry).")
    ...
    # 5B) No fresh latest signal: backfill previous bar if it looks missed
        ...
        if prev_signal and (last_signal != prev_signal or open_position != prev_signal):
            if not pending_signal:
                print(f"[BACKFILL] BTC-5M: previous bar had {prev_signal} — attempting execution now.")
                ...
                if ok:
                    ...
                    pending_since = now_local
                    print(f"[PENDING] BTC-5M: queued {prev_signal} from previous bar (will retry).")
        else:
