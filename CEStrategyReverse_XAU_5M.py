"""
Chandelier Exit Strategy for XAUUSD (Reverse)
Using Heikin-Ashi Candles
XAU - 5 Minute TF
"""

# ===== FILENAME: CEStrategyReverse_XAU_5M =====
...
atr_period = 1
atr_mult = 1.85
magic_number = 223344  # Unique ID for this EA's trades
local_tz = timezone('Europe/London')

# === RISK & STOPS CONFIG === #
risk_per_trade_pct = 0.25      # percent of balance risked per trade
sl_usd_distance    = 2.5       # fixed $ distance from entry


# === SESSION FILTER (Europe/London local time) === #
session_windows = [
    ("00:30", "05:30"),  # Pre-London / Asian hours (choppy/mean-reverting)
    ("18:30", "22:00"),  # After overlap fades until late US evening
]
...
mt5_login = 52474878
mt5_password = "uPkx&T&1F5VyUM"
mt5_server = "ICMarketsSC-Demo"
mt5_terminal_path = r"C:\MT5\52474878\terminal64.exe"
...
def _maybe_retry_pending():
    ...
    print(f"[RETRY] XAU-5M-REV: attempting pending {pending_signal} execution...")
    ok = _attempt_execution_for_signal(pending_signal)
    last_retry_at = now_local
    if ok:
        print(f"[OK] XAU-5M-REV: pending {pending_signal} executed.")
        pending_signal = None
        pending_since  = None
        last_retry_at  = None
...
while True:
    ...
    if signal:
        ...
        if pending_signal and signal != pending_signal:
            print(f"[CANCELLED] XAU-5M-REV: live signal {signal} overrides pending {pending_signal}.")
            ...
            if signal != last_signal or open_position != signal:
                ...
                print(f"[TRADE] XAU-5M-REV: new signal={signal} | prev={prev_sig} | open={open_position or 'NONE'}")
                last_signal = signal
                ok = _attempt_execution_for_signal(signal)
                if ok:
                    pending_signal = None
                    ...
                    if not pending_signal:
                        pending_signal = signal
                        pending_since = now_local
                        print(f"[PENDING] XAU-5M-REV: queued {signal} execution (will retry).")
    ...
    # 5B) No fresh latest signal: backfill previous bar if it looks missed
        ...
        if prev_signal and (last_signal != prev_signal or open_position != prev_signal):
            if not pending_signal:
                print(f"[BACKFILL] XAU-5M-REV: previous bar had {prev_signal} — attempting execution now.")
                ...
                if ok:
                    ...
                    pending_since = now_local
                    print(f"[PENDING] XAU-5M-REV: queued {prev_signal} from previous bar (will retry).")
        else:
